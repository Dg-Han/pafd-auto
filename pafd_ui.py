...
def mouseClick(clicktimes,LorR,img,rpt_t=1):
    ...
    while True:
        ...
        if location is not None:
            ...
            js+=1
            if js==50:
                print('Fail to click for more than 5 seconds, please check if there is a problem...')
        time.sleep(0.1)
...
def idfy_by_chars(img_path,resize=0):
    ...
    if len(kp1)<5:
        ...
        '''
        good=[m for m,n in matches if (m.distance<0.7*n.distance) or (m.distance<150)]
        print(len(good),len(kp1))
        
        '''
        cv2.Dmatch类包含以下属性
        distance | 特征距离, 0表示完全一致
        trainIdx | 目标图像中特征点索引
        queryIdx | 查询图像中特征点索引
        imgIdx | 目标图像索引
        '''
...
class Ui(Frame):
    ...
    def createWidgets(self):
        ...
        self.btn=Button(self.top,text='Begin',command=lambda: self.run(0))
        self.btn.place(relx=0.4,rely=0.1,relwidth=0.2,relheight=0.1)

        self.btn1=Button(self.top,text='Skip',command=lambda: self.run(1))
...
class pafd(Ui):
    ...
    def run(self,skip):
        ...
        if cmd_set:
            ...
            self.txt.master.update()
            del cmd_set[0]
            if cmd_set:
                self.run(cmd_set)
    
    def mouseClick(self,clicktimes,LorR,img,rpt_t=1):
        i=1
        js=0
        while True:
            location = pyautogui.locateCenterOnScreen(img,confidence=0.9)
            ...
            if location is not None:
                ...
                js+=1
                if js==50:
                    self.refresh('Fail to click for more than 5 seconds, please check if there is a problem...')
            self.txt.master.update()
